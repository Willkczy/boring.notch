...
//
//  Conversation/transcript view. Ported from vibe-notch with three adaptations:
//   - binds to our `AgentSessionMonitor` (not vibe's `ClaudeSessionMonitor`)
//   - back-navigation + terminal-focus go through injected closures
//     (`onBack` / `onFocus`), replacing vibe's NotchViewModel
//   - tmux message-sending is gutted (boringNotch doesn't drive tmux); the
//     input bar stays disabled, exactly as vibe shows for non-tmux sessions
//  History comes from the self-contained ChatHistoryManager port.
//
...
struct ChatView: View {
    ...
    var onBack: () -> Void
    /// Bring the session's host terminal/app forward.
    var onFocus: () -> Void
    ...
    var body: some View {
        ...
        .onReceive(sessionMonitor.$instances) { sessions in
            ...
            } else if !sessions.contains(where: { $0.sessionId == sessionId }) && hasLoadedOnce {
                // Session vanished (ended/archived) — return to the list.
                onBack()
            }
        }
    }
    ...
    private var messageList: some View {
        ScrollViewReader { proxy in
            ...
            .scaleEffect(x: 1, y: -1)
            // Autoscroll-pause detection needs onScrollGeometryChange (macOS 15+).
            // On macOS 14 this no-ops: autoscroll simply stays on (acceptable
            // degradation; boringNotch's deployment target is 14.0).
            .scrollAtBottomChange { wasAtBottom, isNowAtBottom in
    ...
    /// Can send messages only if session is in tmux. boringNotch does not drive
    /// tmux, so this is always false — the input bar stays disabled (same as
    /// vibe-notch shows for any non-tmux session).
    ...
    private func sendMessage() {
        ...
        guard !text.isEmpty, canSendMessages else { return }

        inputText = ""

        // Resume autoscroll when the user sends a message.
        resumeAutoscroll()
        shouldScrollToBottom = true

        // Don't add to history here — it syncs from the JSONL once the agent
        // records the prompt (the transcript watcher reparses).
        Task {
            await sendToSession(text)
        }
    }

    /// Inject the message into the session's tmux pane via send-keys.
    ...
    /// Map a pane tty (e.g. `/dev/ttys003`) to its tmux target by scanning all
    /// panes. Ported from vibe-notch; normalizes `/dev/` so either the bare or
    /// full tty form matches.
    private func findTmuxTarget(tty: String) async -> TmuxTarget? {
        guard let tmuxPath = await TmuxPathFinder.shared.getTmuxPath() else {
            return nil
        }

        let wantTty = tty.replacingOccurrences(of: "/dev/", with: "")
...
// MARK: - Scroll-bottom detection (macOS 15+ only)

private extension View {
    /// Reports whether the inverted scroll view is near its visual bottom.
    /// Uses `onScrollGeometryChange` on macOS 15+, no-ops on 14 (boringNotch's
    /// deployment target), where autoscroll just stays enabled.
    @ViewBuilder
    func scrollAtBottomChange(_ action: @escaping (_ wasAtBottom: Bool, _ isNowAtBottom: Bool) -> Void) -> some View {
        if #available(macOS 15.0, *) {
            self.onScrollGeometryChange(for: Bool.self) { geometry in
                geometry.contentOffset.y < 50
            } action: { wasAtBottom, isNowAtBottom in
                action(wasAtBottom, isNowAtBottom)
            }
        } else {
            self
        }
