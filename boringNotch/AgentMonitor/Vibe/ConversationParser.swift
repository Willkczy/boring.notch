...
//
//  Parses Claude JSONL conversation files to extract summary, last message, and
//  the full chat history (incremental). Ported verbatim from vibe-notch except:
//   - UsageInfo / ConversationInfo moved to ConversationModels.swift
//   - logger subsystem renamed to boringNotch's
//   - added `parseInfo(filePath:)` — boringNotch's hook payload gives us the
//     transcript path directly, so we can parse by path without deriving it
//     from cwd+sessionId (path derivation is kept for the chat/subagent paths).
//
...
actor ConversationParser {
    ...
    nonisolated static let logger = Logger(subsystem: "com.notchagent.app", category: "Parser")
    ...
    func parse(sessionId: String, cwd: String) -> ConversationInfo {
        ...
        return parseInfo(filePath: sessionFile)
    }

    /// Parse conversation info directly from a transcript file path. boringNotch
    /// addition: the hook payload supplies `transcript_path` so we don't need to
    /// derive the path from cwd+sessionId.
    func parseInfo(filePath sessionFile: String) -> ConversationInfo {
    ...
    func parseFullConversation(sessionId: String, cwd: String) -> [ChatMessage] {
        let sessionFile = Self.sessionFilePath(sessionId: sessionId, cwd: cwd)
        return parseFullConversation(sessionId: sessionId, filePath: sessionFile)
    }

    /// Parse full conversation by explicit transcript path (boringNotch addition).
    func parseFullConversation(sessionId: String, filePath sessionFile: String) -> [ChatMessage] {
    ...
    func parseIncremental(sessionId: String, cwd: String) -> IncrementalParseResult {
        let sessionFile = Self.sessionFilePath(sessionId: sessionId, cwd: cwd)
        return parseIncremental(sessionId: sessionId, filePath: sessionFile)
    }

    /// Incremental parse by explicit transcript path (boringNotch addition).
    func parseIncremental(sessionId: String, filePath sessionFile: String) -> IncrementalParseResult {
        guard FileManager.default.fileExists(atPath: sessionFile) else {
